...
void _STDCALL PrintInfo(_PPMD_FILE* DecodedFile, _PPMD_FILE* EncodedFile) {
	...
	sprintf(WrkStr, "%14s:%7d >%7d, %1d.%02d bpb, used:%3d.%1dMB, speed: %d KB/sec", pFName, NDec, NEnc, n1, n2, m1, m2, Speed);
...
inline void EncodeFile(const ENV_FIND_RESULT& efr, int MaxOrder, int SASize, MR_METHOD MRMethod, const char* ArcName) {
	...
	ai.FNLen = strlen(pFName) + (MRMethod << 14);
	...
	EncodeFile(fpOut, fpIn, MaxOrder, MRMethod);
	putchar('\n');
...
inline BOOL DecodeOneFile(FILE* fpIn) {
	...
	DecodeFile(fpOut, fpIn, MaxOrder, MRMethod);
	putchar('\n');
...
inline void TestArchive(char* ArcName, const char* Pattern) {
	...
	if (fp) {
		if (!fread(&ai, sizeof(ai), 1, fp) || ai.signature != PPMdSignature || (ai.info >> 12) + 'A' != ::Variant) {
...
int main(int argc, char* argv[]) {
	...
	if (argc < 3) {
		printf(MTxt[6], SASize, MAX_O, MaxOrder);
		return -1;
	}
	...
	for (ArcName[0] = 0, i = 2; i < argc && (argv[i][0] == '-' || argv[i][0] == '/'); i++) {
		switch (toupper(argv[i][1])) {
			...
			case 'F':
				...
				break;
			case 'M':
				SASize = CLAMP(atoi(argv[i] + 2), 1, 256);
				break;
			case 'O':
				MaxOrder = CLAMP(atoi(argv[i] + 2), 2, MAX_O);
				break;
			case 'R':
				MRMethod = MR_METHOD(CLAMP(atoi(argv[i] + 2), 0, 2));
				break;
			default:
	...
	while ((pNode = pFirstNode) != NULL) {
		...
		if (EncodeFlag) {
			EncodeFile(efr, MaxOrder, SASize, MRMethod, ArcName);
		} else {
			DecodeFile(efr);
		}
		if (DeleteFile) {
