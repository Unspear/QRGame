...
atlas = [[0]*(width*16*2) for y in range(height*16)]
index = 0
unidata_width = {}
for code in random.sample(sorted(unidata), k=len(unidata)):
    value = unidata[code]
    ax = index % width
    ay = index // width
    if ay >= height:
    ...
    for cy in range(16):
        ...
        for cx in range(16):
            atlas[ay*16+cy][(ax*16+cx)*2] = 0 #Luminance
            atlas[ay*16+cy][(ax*16+cx)*2+1] = int(crow[cx]) #Alpha
    index += 1

# Create and save chars
png.from_array(atlas, 'LA;1').save("poster.png")
